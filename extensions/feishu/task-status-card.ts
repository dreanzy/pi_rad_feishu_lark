...
export type TaskStatusSink = {
  ...
  finish(status: Exclude<TaskStatus, "running">, phase?: string): Promise<void>;
};
...
export class TaskStatusCard implements TaskStatusSink {
  ...
  async finish(status: Exclude<TaskStatus, "running">, phase?: string) {
    await this.finishFinal(status, phase, false);
  }

  private async finishFinal(status: Exclude<TaskStatus, "running">, phase: string | undefined, force: boolean) {
    if (this.status !== "running") return;
    this.status = status;
    this.version += 1;
    this.stopHeartbeat();
    ...
    await this.patch(buildTaskStatusCard({ key: this.key, status, phase: finalPhase }), { final: true, force });
  }
  ...
  private async flushRunningUpdate() {
    ...
    try {
      await this.patch(buildTaskStatusCard({ key: this.key, status: "running", phase: this.phase }), { version });
    } finally {
    ...
    const next = this.patchQueue
      ...
      .then(async () => {
        ...
        try {
          ...
          debugLog("feishu.task_status.update_error", { error: error instanceof Error ? error.message : String(error) });
  ...
  private async retryFinalPatch(messageId: string, card: object) {
    ...
    try {
      ...
      debugLog("feishu.task_status.final_retry_error", { error: error instanceof Error ? error.message : String(error) });
    }
  }
    ...
    elements: [
      ...
      ...(running ? [{
        ...
        actions: [{
          ...
          type: "danger",
          value: { action: STOP_ACTION, key: input.key },
        }],
      }] : []),
    ],
  };
...
export function parseStopTaskActionValue(value: unknown): { key: string } | undefined {
...
function defaultFinalPhase(status: Exclude<TaskStatus, "running">): string | undefined {
