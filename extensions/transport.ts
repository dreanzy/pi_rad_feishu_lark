...
export class FeishuTransport {
	...
	private async probeBotOpenId() {
		try {
			...
			if (!this.botOpenId) {
				throw new Error(
					`bot/v3/info response missing open_id: ${JSON.stringify(res).slice(0, 200)}`,
				);
			}
		} catch (error) {
	...
	private async handleRawMessage(data: any) {
		...
		if (
			...
			if (!this.isMentioned(message)) {
				debugLog("feishu.message.ignored_not_mentioned", {
					messageId: message.message_id,
				});
				return;
			}
	...
	private isMentioned(message: any): boolean {
		...
		return mentions.some(
			(m: any) => m?.id?.open_id === botOpenId || m?.id?.union_id === botOpenId,
		);
	}
	...
	private async getChatMode(
		...
		try {
			const res = await this.sdkClient.im.v1.chat.get({
				path: { chat_id: chatId },
			});
	...
	private buildMarkdownCardPartsWithCopySources(text: string) {
		return buildMarkdownCardParts(text, this.config.language, () =>
			this.createMarkdownCopySourceId(),
		).map((part) => {
			const copySourceId = extractCopySourceId(part.card);
			if (copySourceId)
				this.rememberMarkdownCopySource(copySourceId, part.markdown);
			return part;
		});
	}
	...
	async downloadMessageResource(
		...
		const mimeType =
			typeof rawContentType === "string"
				...
				: undefined;
		debugLog("feishu.download.resource.done", {
			messageId,
			fileKey,
			type,
			bytes: bytes.length,
	...
	async downloadImage(
		...
		const bytes = await streamToBuffer(readableFromDownload(result));
		debugLog("feishu.download.image.fallback_done", {
			messageId,
			imageKey,
			bytes: bytes.length,
		});
...
function extractCopySourceId(card: object) {
	...
	for (const element of elements) {
		...
		for (const behavior of behaviors) {
			...
			if (
				value?.action === "pi_feishu_copy_markdown" &&
				typeof value.copySourceId === "string"
			) {
...
function readableFromDownload(result: any): NodeJS.ReadableStream {
	return typeof result?.getReadableStream === "function"
